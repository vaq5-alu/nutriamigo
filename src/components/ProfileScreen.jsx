...
export default function ProfileScreen({ initialData, weightHistory, streak, onSave, onBack, onLogout, onDeleteAccount, onCancelPremium, onReactivatePremium }) {
    ...
    <div className="max-w-4xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 pb-10">
        ...
        <div className="lg:col-span-1 space-y-6">
          <div className="bg-white p-8 rounded-[2.5rem] shadow-xl shadow-gray-200/50 border border-gray-100 text-center relative overflow-hidden group">
            <div className="absolute top-0 left-0 w-full h-2 bg-emerald-500"></div>
            <div className="w-28 h-28 bg-emerald-50 rounded-3xl mx-auto flex items-center justify-center text-5xl mb-6 shadow-inner transform group-hover:scale-110 transition-transform duration-300 overflow-hidden">
            ...
            <h3 className="text-2xl font-black text-gray-900 italic uppercase">Nivel {profile.level || 1}</h3>
            <div className="flex items-center justify-center gap-2 mt-1">
               <span className="text-emerald-600 font-bold text-lg">{profile.xp || 0}</span>
               <span className="text-gray-400 font-bold text-sm tracking-widest uppercase">XP TOTAL</span>
            </div>
            
            <div className="w-full bg-gray-100 rounded-full h-3 mt-6 p-0.5 shadow-inner">
              <div 
                className="bg-gradient-to-r from-emerald-500 to-emerald-400 h-2 rounded-full shadow-[0_0_10px_rgba(16,185,129,0.4)] transition-all duration-1000" 
                style={{ width: `${(profile.xp || 0) % 100}%` }}
              ></div>
            </div>
            <p className="text-[10px] text-gray-400 font-bold mt-3 tracking-widest uppercase">Faltan {100 - ((profile.xp || 0) % 100)} XP para el siguiente nivel</p>
          </div>

          <div className="bg-emerald-600 p-6 rounded-[2rem] text-white shadow-lg shadow-emerald-200 relative overflow-hidden">
             <div className="absolute -right-4 -bottom-4 w-32 h-32 bg-white/10 rounded-full blur-2xl"></div>
             <h4 className="font-bold opacity-80 text-xs uppercase tracking-widest mb-1">Racha</h4>
             <p className="text-3xl font-black tracking-tighter">{streak} Días 📅</p>
             <p className="text-[10px] mt-2 opacity-70">{streak > 0 ? "¡Sigue así!" : "¡Empieza hoy!"}</p>
          </div>
        </div>
          ...
          <form onSubmit={handleSubmit} className="bg-white rounded-[2.5rem] shadow-xl shadow-gray-200/50 border border-gray-100 p-10 space-y-10">
            ...
            <section className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
              ...
              {(onLogout || onDeleteAccount) && (
                <section className="space-y-6 pt-10 mt-10 border-t border-gray-100">
              ...
              <section className="space-y-8 animate-in fade-in slide-in-from-right-4 duration-300">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                   <div className="p-5 bg-white border border-gray-100 rounded-3xl shadow-sm">
                      ...
                      <p className="text-lg font-black text-emerald-600">NutriCoach Pro</p>
                      <p className="text-xs text-gray-400 mt-0.5">Mensual • 0,00€/mes</p>
                   </div>
                   <div className="p-5 bg-white border border-gray-100 rounded-3xl shadow-sm">
                      ...
                      <div className="flex items-center gap-2">
                         ...
                         <p className="text-lg font-black text-gray-900">{profile.is_premium ? 'Activo' : 'Inactivo'}</p>
                      </div>
                   </div>
                   <div className="p-5 bg-white border border-gray-100 rounded-3xl shadow-sm">
                      <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">Próximo Evento</p>
                      <p className="text-lg font-black text-gray-900">{profile.premium_until ? new Date(profile.premium_until).toLocaleDateString() : 'N/A'}</p>
                   </div>
                </div>
                ...
                <div className="space-y-4">
                   <h5 className="font-bold text-gray-900 ml-2 italic">Historial de Facturación (Simulado)</h5>
                   <div className="bg-white border border-gray-100 rounded-3xl overflow-hidden shadow-sm">
                      <table className="w-full text-left text-sm">
                         ...
                         <tbody className="divide-y divide-gray-50">
                            {profile.is_premium && (
                               <tr>
                                  <td className="px-6 py-4 font-medium text-gray-600">{new Date().toLocaleDateString()}</td>
                                  <td className="px-6 py-4 font-bold text-gray-900">Suscripción Mensual</td>
                                  <td className="px-6 py-4 font-black text-gray-900">0,00€</td>
                                  <td className="px-6 py-4"><span className="bg-emerald-100 text-emerald-700 px-2 py-0.5 rounded-full text-[10px] font-bold">PAGADO</span></td>
                               </tr>
                            )}
                         </tbody>
                      </table>
            ...
            <div className="space-y-10 animate-in fade-in slide-in-from-right-4 duration-300">
              ...
              {!isProfileCompleteLocal && (
                <div className="p-6 bg-emerald-50 border-2 border-emerald-100 rounded-[2rem] flex items-start gap-4 shadow-sm">
              ...
              <section className="space-y-6">
                ...
                <div className="grid grid-cols-2 gap-6">
                  <div className="space-y-2">
                    <label className="text-xs font-bold text-gray-400 uppercase tracking-widest ml-1">
                      Edad {!profile.age && <span className="text-red-500 font-black ml-1">● Requerido</span>}
                    </label>
                    <input
                      type="number"
                      name="age"
                      value={profile.age || ''}
                      onChange={handleChange}
                      className={`w-full px-5 py-3.5 bg-gray-50 border-2 rounded-[1.25rem] focus:bg-white focus:ring-4 outline-none transition-all font-bold text-gray-800 ${!profile.age ? 'border-amber-200 bg-amber-50/20' : 'border-transparent'}`}
                      placeholder="Ej: 25"
                    />
                  </div>
                  <div className="space-y-2">
                    <label className="text-xs font-bold text-gray-400 uppercase tracking-widest ml-1">
                      Género {!profile.gender && <span className="text-red-500 font-black ml-1">● Requerido</span>}
                    </label>
                    <select 
                      name="gender" 
                      value={profile.gender || ''} 
                      onChange={handleChange}
                      className={`w-full px-5 py-4 bg-gray-50 border-2 rounded-[1.25rem] focus:bg-white focus:ring-4 outline-none transition-all font-bold text-gray-800 appearance-none ${!profile.gender ? 'border-amber-200 bg-amber-50/20' : 'border-transparent'}`}
                    >
                ...
                <div className="grid grid-cols-2 gap-6">
                  <div className="space-y-2">
                    <label className="text-xs font-bold text-gray-400 uppercase tracking-widest ml-1">
                      Peso Actual (kg) {!profile.current_weight && <span className="text-red-500 font-black ml-1">● Requerido</span>}
                    </label>
                    <input
                      type="number"
                      step="0.1"
                      ...
                      onChange={handleChange}
                      className={`w-full px-5 py-3.5 bg-gray-50 border-2 rounded-[1.25rem] focus:bg-white focus:ring-4 outline-none transition-all font-bold text-gray-800 ${!profile.current_weight ? 'border-amber-200 bg-amber-50/20' : 'border-transparent'}`}
                      placeholder="0.0"
                    />
                  </div>
                  <div className="space-y-2">
                    <label className="text-xs font-bold text-gray-400 uppercase tracking-widest ml-1">
                      Altura (cm) {!profile.height && <span className="text-red-500 font-black ml-1">● Requerido</span>}
                    </label>
                    <input 
                      type="number" 
                      name="height" 
                      ...
                      onChange={handleChange} 
                      className={`w-full px-5 py-3.5 bg-gray-50 border-2 rounded-[1.25rem] focus:bg-white focus:ring-4 outline-none transition-all font-bold text-gray-800 ${!profile.height ? 'border-amber-200 bg-amber-50/20' : 'border-transparent'}`}
                      placeholder="Ej: 175"
                    />
                  </div>
                </div>
                ...
                <div className="space-y-2">
                  <label className="text-xs font-bold text-emerald-600/60 uppercase tracking-widest ml-1">
                    Peso Objetivo (kg) {!profile.target_weight && <span className="text-red-500 font-black ml-1">● Requerido</span>}
                  </label>
                  <div className="relative">
                     <input 
                        ...
                        onChange={handleChange} 
                        className={`w-full px-6 py-4 bg-emerald-50/50 border-2 rounded-[1.5rem] focus:bg-white focus:ring-4 focus:ring-emerald-500/20 focus:border-emerald-500 outline-none font-black text-2xl text-emerald-900 ${!profile.target_weight ? 'border-amber-300' : 'border-emerald-100'}`} 
                        placeholder="70.0"
                      />
                     <div className="absolute right-6 top-1/2 -translate-y-1/2 text-emerald-300 font-black text-xl">TARGET</div>
                  </div>
                </div>
              </section>
              ...
              <section className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="space-y-2">
                    <label className="text-xs font-bold text-gray-400 uppercase tracking-widest ml-1">
                      Tu Objetivo {!profile.goal && <span className="text-red-500 font-black ml-1">● Requerido</span>}
                    </label>
                    <select 
                      name="goal" 
                      value={profile.goal || ''} 
                      onChange={handleChange} 
                      className={`w-full px-5 py-3.5 bg-gray-50 border-2 rounded-[1.25rem] focus:bg-white focus:ring-4 outline-none font-bold text-gray-800 appearance-none ${!profile.goal ? 'border-amber-200 bg-amber-50/20' : 'border-transparent'}`}
                    >
                      <option value="">-- Selecciona --</option>
                  ...
                  <div className="space-y-2">
                    ...
                    <input type="number" name="daily_calories" value={profile.daily_calories || ''} onChange={handleChange} className="w-full px-5 py-3.5 bg-gray-50 border-transparent rounded-[1.25rem] focus:bg-white focus:ring-4 focus:ring-emerald-500/10 focus:border-emerald-500 outline-none transition-all font-bold text-gray-800" placeholder="Ej: 2000" />
                  </div>
                </div>
              </section>
            </div>
            ...
            <div className="pt-10">
              <button
                ...
                className={`w-full flex justify-center py-5 px-4 rounded-[1.5rem] shadow-xl text-lg font-black text-white transition-all outline-none active:scale-95 ${
